...
export function synthesizePlanFromConfig(
  ...
  if (hasEmail) {
    motions.push({
      ...
      rationale: `Signal-based cold email to ${config.icp.titles?.join(", ") || "ICP titles"} in ${config.geo || config.icp.geo || "target geo"}. Hooks tie to recent funding, hiring, or launch signals.`,
  ...
  if (motions.length === 0) {
    motions.push({
      ...
      rationale: "Default email outbound motion.",
      primary_channel: "email",
    });
  }
  ...
  return {
    ...
    channel_rationale: hasEmail
      ? `Email is primary for ${config.icp.industries?.join(", ") || "target industries"} — signal hooks lift reply rates. ${hasX ? "X supplements Tier 1 with public-activity personalization." : ""}`
      : "X-only motion — individually reviewed, no bulk DMs.",
    risks: [
      "Deliverability depends on sender DNS (SPF/DKIM/DMARC)",
      "Thin signal coverage may limit Tier 1 volume",
      ...(config.exclusions?.length ? [`Exclusions may shrink addressable market: ${config.exclusions.length} rules`] : []),
    ],
    prerequisites: [
